...
echo [7] Hapus Klik Kanan Desktop  (Non-LTSC)
...
if "%menu%"=="7" if /i "%OSMode%"=="Non-LTSC" goto REMOVE_CONTEXT
...
set "REMOTE_FILE_URL=https://raw.githubusercontent.com/r3ndycom/wucontrol/main/WUControl.cmd"
set "TMP_DL=%TEMP%\WUControl_update.cmd"
set "THIS_CMD=%~f0"
...
echo [PERINGATAN] Update tersedia. Mendownload...
...
:: Buat pembantu ganti file karena file sedang berjalan
set "REPL_BAT=%TEMP%\replace_and_run.bat"
(
    echo @echo off
    ...
    echo echo [OK] WUControl.cmd berhasil diperbarui.
    echo start "" "%THIS_CMD%"
) > "%REPL_BAT%"

echo Menjalankan WUControl versi terbaru...
timeout /t 5 >nul
start "" "%REPL_BAT%"
