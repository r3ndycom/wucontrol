...
:: URL GitHub raw
set "REMOTE_MD5_URL=https://raw.githubusercontent.com/r3ndycom/wucontrol/main/WUControl.md5"
set "REMOTE_FILE_URL=https://raw.githubusercontent.com/r3ndycom/wucontrol/main/WUControl.bat"

:: Ambil MD5 dari server
...
:: List target update
set TARGETS[0]=%SCRIPT_FIXED%
set TARGETS[1]=%SCRIPT_CURRENT%

:: Loop tiap target
for %%T in ("%TARGETS[0]%" "%TARGETS[1]%") do (
    set "THIS_CMD=%%~T"
    ...
    echo.
    echo Target: !THIS_CMD!
    echo Local  MD5: !LOCAL_MD5!
    
    if /i "!LOCAL_MD5!"=="%REMOTE_MD5%" (
        echo [OK] Script sudah paling baru di !THIS_CMD!
    ) else (
        echo [PERINGATAN] Update tersedia di !THIS_CMD!
        ...
        if /i not "!DL_MD5!"=="%REMOTE_MD5%" (
            echo [PERINGATAN] MD5 hasil download TIDAK cocok. Abort !THIS_CMD!
            del /f /q "%TMP_DL%"
        ) else (
            :: Buat pembantu ganti file
            set "REPL_BAT=%TEMP%\replace_!RANDOM!.bat"
            (
                echo @echo off
                echo timeout /t 1 >nul
                echo copy /y "%TMP_DL%" "!THIS_CMD!" >nul 2^>^&1
                echo del /f /q "%TMP_DL%" >nul 2^>^&1
                echo echo [OK] Update selesai: !THIS_CMD!
            ) > "!REPL_BAT!"
            start "" "!REPL_BAT!"
        )
    )
